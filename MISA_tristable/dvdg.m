...
for i = 1:N
    ...
    b(i) = -1+sum((C(:,i)+D(:,i))'.*mu2.^4./(S(:,i)'.^4+mu2.^4));
    for j = setdiff(1:N,i)
...
for i = 1:N
    ...
    b(i) = -1+sum((C(:,i)+D(:,i))'.*mu3.^4./(S(:,i)'.^4+mu3.^4));
    for j = setdiff(1:N,i)
