function dlossdg = KLlossdg(datamu1,datamu2,datamu3,datasigma1,datasigma2,datasigma3,...
                   sigma1,sigma2,sigma3,dSigma1dg,dSigma2dg,dSigma3dg,...
                   dv1dg,dv2dg,dv3dg,mu1,mu2,mu3,alpha,dataweight) 
...
dloss1dg = zeros(size(mu1,2),1); dloss2dg = zeros(size(mu1,2),1); dloss3dg = zeros(size(mu1,2),1);
 ...
 dloss1dg(i) = 1/2*(alpha(1)+dataweight(1))*(mu1-datamu1)*(datasigma1\dv1dg(:,i))+1/2*(alpha(2)+dataweight(2))*(mu2-datamu2)*(datasigma2\dv2dg(:,i))...
     +1/2*(alpha(3)+dataweight(3))*(mu3-datamu3)*(datasigma3\dv3dg(:,i));
 for ee = 1:size(mu1,2)
     sigma0 = sigma1;
