function [dSigmads] = dsigmads(g,G,k,mu,S,C,D,N,sigma1,dv1ds,Jacob1,dd)
% the last dimension indicates g, the first two indicate sigma
dSigmads = zeros(N,N,N);
dsdfdx = ds_dfdx(g,G,mu,S,C,D,N,dv1ds);
for i = 1:N
    A = zeros(N*N);
    b = zeros(N*N,1);
    for pp = 1:N
    ...
    for aa = 1:N
    b(aa*N-N+aa) = -sigma1(aa,:)*dsdfdx(aa,:,i)';
        for mm = setdiff(1:N,aa)
            b(aa*N-N+mm) = -sigma1(aa,:)*dsdfdx(mm,:,i)'-sigma1(mm,:)*dsdfdx(aa,:,i)';
        end      
    end
