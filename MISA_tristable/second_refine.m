...
datasigma(:,:,3) = [0.4087	-0.0007;-0.0007	0.4000];%ground_truth sigma
...
for sample=15:20  %1:20 results in longer computation time
    G = [16+randno(sample)*2/3 16+randno(sample+50)*2/3]'; 
    ...
    S = [4 4;5 5]; % can be perturbed, here to avoid phase transitions

    wemax = 10*ones(1,N);
    lambda_penalty = 0;%more accurate fit to parameters, rather than weights
    ...
    for iter = 1:itermax
        ...
        if size(mu,1)==3
            ...
            [dv1dg,dv2dg,dv3dg] = dvdg(g,G,k,mu(1,:),mu(2,:),mu(3,:),S,C,D,N);
            ...
            dSigma1dg=dsigmadg(g,G,k,mu(1,:),S,C,D,N,sigma(:,:,1),dv1dg,Jacob1,dd); 
            ...
            dlossdg = KLlossdg(datamu1,datamu2,datamu3,datasigma(:,:,1),datasigma(:,:,2),datasigma(:,:,3),...
            ...
            lambdad = 10^(-floor(log10(max(abs(dlossdd(:)))))-2.5);
             ...
             ourloss(no,sample) = loss;
             ourmu{no,sample} = mu; oursigma{no,sample} = sigma;
             ourg{no,sample} = g; ourG{no,sample} = G; ourd(no,sample) = dd; ourS{no,sample} = S;
             no = no+1;
        else
