function dlossdg = KLlossdgnew(datamu1,datamu2,datasigma1,datasigma2,...
                   sigma1,sigma2,dSigma1dg,dSigma2dg,...
                   dv1dg,dv2dg,mu1,mu2,alpha,dataweight,dalphadg,lambda_penalty) 

dlossdg = zeros(size(mu1,2),1);
dloss1dg = zeros(size(mu1,2),1); dloss2dg = zeros(size(mu1,2),1); dloss3dg = zeros(size(mu1,2),1);
dloss4dg = zeros(size(mu1,2),1); 
N = size(mu1,2);
