...
params.Dimension = 11; %The dimension of the system.
...
N=5;  %%the dimension of the system
xx=zeros(cycle_index,N);
rng('default')
%%Solve odes from different initial values
for i=1:cycle_index
    x0=rand(1,N).*wemax;
    [t,x]=ode15s(@(t,x)force5(t,x,k,G,g,S,IM,Hilln,AB,C,D),[0,100],x0);
    newx=x(end,:);
    x=inf*ones(1,N);
    while norm( x(end,:)-newx(end,:) ,2 )>1e-5
        x=newx;
        [t,newx]=ode15s(@(t,x)force5(t,x,k,G,g,S,IM,Hilln,AB,C,D),[0,100],x(end,:));
    end
    xx(i,:)=newx(end,:);
end
