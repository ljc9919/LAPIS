function dlossdS = KLlossdsnew(datamu1,datamu2,datasigma1,datasigma2,...
                   sigma1,sigma2,dSigma1dS,dSigma2dS,...
                   dv1dS,dv2dS,mu1,mu2,alpha,dataweight,dalphadS,lambda_penalty)

dlossdS = zeros(size(mu1,2),1);
dloss1dS = zeros(size(mu1,2),1); dloss2dS = zeros(size(mu1,2),1); dloss3dS = zeros(size(mu1,2),1);
dloss4dS = zeros(size(mu1,2),1); 
N = size(mu1,2);
 ...
 for ee = 1:size(mu1,2)
  ...
  dloss4dS(i) = lambda_penalty*sum((alpha-dataweight).*dalphadS(:,i));
  
  dlossdS(i) = dloss1dS(i)+dloss2dS(i)+dloss3dS(i)+dloss4dS(i);
